...
{
    public class DeleteCardsCommandHandler : IRequestHandler<DeleteCardsCommand, Result<CardClosureResponseModel>>
    {
        private readonly ILogger<DeleteCardsCommandHandler> _logger;
        ...
        public DeleteCardsCommandHandler(ILogger<DeleteCardsCommandHandler> logger, ICardServiceDbContext dbContext,
        ...
        public async Task<Result<CardClosureResponseModel>> Handle(DeleteCardsCommand request,
            CancellationToken cancellationToken)
        {
            ...
            if (card is null)
            {
                ...
                _logger.LogInformation($"{nameof(DeleteCardsCommandHandler)} :: {result}");
                return result;
            }

            /* var validateCardPinResult = await _cardPinService.ValidateCardPin(request.Pin, card, cancellationToken);
             if (validateCardPinResult.IsSuccess is false)
             {
                 ...
                 _logger.LogInformation($"{nameof(DeleteCardsCommandHandler)} :: {result.ToJson()}");
                 return result;
             } */

            var validateOtp = await _otpService.ValidateOtp(request.UserName, request.Otp, cancellationToken);
            if (validateOtp.IsSuccess == false) return Result.Fail<CardClosureResponseModel>("Invalid OTP");
            ...
            result = await cardManagementService.CloseCard(
                new CardClosureRequestModel {Card = card, Reason = request.Reason}, cancellationToken);
