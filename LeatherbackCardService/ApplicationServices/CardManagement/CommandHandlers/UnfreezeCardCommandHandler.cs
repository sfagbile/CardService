...
{
    public class UnfreezeCardCommandHandler : IRequestHandler<UnfreezeCardCommand, Result<CardActivationResponseModel>>
    {
        private readonly ILogger<UnfreezeCardCommandHandler> _logger;
        ...
        public UnfreezeCardCommandHandler(ILogger<UnfreezeCardCommandHandler> logger, ICardServiceDbContext dbContext,
        ...
        public async Task<Result<CardActivationResponseModel>> Handle(UnfreezeCardCommand request,
            CancellationToken cancellationToken)
        {
            _logger.LogInformation($"{nameof(UnfreezeCardCommandHandler)} :: Request: {request.ToJson()}");
            ...
            if (card is null)
            {
                ...
                _logger.LogInformation($"{nameof(UnfreezeCardCommandHandler)} :: {result.ToJson()}");
                return result;
            }
            ...
            if (validateCardPinResult.IsSuccess is false)
                return Result.Fail<CardActivationResponseModel>("Invalid Pin");
